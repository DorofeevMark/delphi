...
from delphi.setup import download, provision, verify_assets
...
class Setup(unittest.TestCase):
    def setUp(self):
        ...
        self.reader = patch("delphi.setup.json.loads", return_value=self.manifest)
        self.reader.start()
    ...
    def test_import_and_reuse_without_download(self):
        with patch("delphi.setup.diagnose", return_value={}) as doctor, patch("delphi.setup.download") as download:
    ...
    def test_failed_diagnostics_does_not_publish(self):
        with patch("delphi.setup.diagnose", side_effect=Failure("broken", "diagnostic failed")):
    ...
    def test_download_is_verified_before_publication(self):
        ...
        with patch("delphi.setup.download", side_effect=fake_download), patch("delphi.setup.diagnose") as doctor:
    ...
    def test_download_process_has_online_environment(self):
        with patch("delphi.setup.subprocess.run") as run:
    ...
    def test_download_process_failure_has_recovery(self):
        with patch("delphi.setup.subprocess.run") as run:
            ...
            with self.assertRaisesRegex(Failure, "delphi setup --from"):
                download(self.root / "download")
...
class Storage(unittest.TestCase):
    def test_platform_defaults(self):
        with patch("delphi.paths.Path.home", return_value=Path("/home/test")), patch("delphi.paths.sys.platform", "darwin"):
            self.assertEqual(data_directory(), Path("/home/test/Library/Application Support/delphi"))
        with patch("delphi.paths.sys.platform", "linux"), patch.dict(os.environ, {"XDG_DATA_HOME": "/tmp/custom-data"}):
            self.assertEqual(data_directory(), Path("/tmp/custom-data/delphi").resolve())
