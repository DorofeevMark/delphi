...
.venv/bin/delphi setup
```

With uv already installed, the shorter local installation is:

```sh
uv tool install --python 3.12 /absolute/path/to/delphi
delphi setup
delphi index -p /absolute/path/to/project
delphi search 'where are user passwords checked?'
```
...
```sh
delphi setup --from /absolute/path/to/all-MiniLM-L6-v2
```

`setup --model /absolute/path` selects a different destination; `DELPHI_MODEL` also overrides it. Setup provisions the pinned MiniLM model only. Custom runtime models remain supported through `--model` or `DELPHI_MODEL`. Index, search, status, and doctor never download assets.
...
- Linux: `$XDG_DATA_HOME/delphi/`, or `~/.local/share/delphi/`

The default model is under `models/all-MiniLM-L6-v2/`; indexes are under `indexes/`. `DELPHI_INDEX_ROOT` overrides the index location. Reinstalling or upgrading the package leaves these directories intact.
