...
Each project has one index in `.delphi`, with CocoIndex incremental state, the SQLite vector table, and a manifest. A file lock prevents reading during updates or concurrent writers. An interrupted/failed update leaves `ready: false`; search refuses it until indexing succeeds. This is not an atomic last-good-snapshot design. To use different model assets or rebuild incompatible state, move `.delphi` aside and run `index` again. `status` reports stored state and does not rescan source files to detect staleness.
